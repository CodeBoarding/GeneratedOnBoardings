...
graph LR
    ...
    B -- routes request --> C(Import Controller)
    C -- orchestrates --> D(ImportProject)
    D -- uses --> E(ImportConfigNormalizer)
    E -- validates --> F{Configuration}
    F -- creates --> G(Bucket)
    G -- defines --> H(FamiliesLoader)
    G -- defines --> I(VariantsLoader)
    H -- loads --> J{Pedigree Data}
    I -- loads --> K{Variant Data}
    K -- annotates --> L(AnnotationPipeline)
    L -- annotates --> M{Annotated Variants}
    M -- describes --> N(PartitionDescriptor)
    N -- partitions --> O{Parquet Files}
    F -- specifies --> P(ImportStorage)
    P -- stores --> O
